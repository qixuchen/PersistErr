...
namespace exact_rev{
    ...
    */
    ...
    int Exact_revised(std::vector<point_t *> p_set, point_set_t *P, int k, int w, int select_opt, int alg_id, int max_round){
        reset_stats();
        start_timer();
        int dim = p_set[0]->dim;
        ...
        int round = 0;
        while(points_return.size() > w && round < max_round){
            point *p1 =0, *p2 =0;
