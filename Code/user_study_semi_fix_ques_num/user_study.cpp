...
int user_study_main_body(){
    ...
    if(jump <= 1){
        // the exact Algorithm
        int alg_id = 0;
        print_alg_start(alg_id);
        exact_rev::Exact_revised(p_set, P0, k, w, SCORE_SELECT, alg_id, 10000);
        print_alg_end(alg_id);
    }
    if(jump <= 2){
        // the sampling Algorithm
        int alg_id = 1;
        print_alg_start(alg_id);
        sampling::sampling(p_set, P0, k, w, SCORE_SELECT, alg_id, 10000);
        print_alg_end(alg_id);
    }
    max_round = std::max(question_asked_list[0], question_asked_list[1]);
    if(jump <= 3){
        // PointPrune Algorithm
        int alg_id = 2;
        print_alg_start(alg_id);
        PointPrune_v2(p_set, P0, 3, w, max, min, alg_id, max_round);
        print_alg_end(alg_id);
    }
    if(jump <= 4){
        // the HDPI Algorithm
        int alg_id = 3;
        print_alg_start(alg_id);
        rev_HDPI(p_set, P0, w, alg_id, max_round);
        print_alg_end(alg_id);
    }
    if(jump <= 5){
        ...
        int alg_id = 4;
        print_alg_start(alg_id);
        Active_Ranking(p_set, P0, w, alg_id, max_round);
        print_alg_end(alg_id);
    }
    if(jump <= 6){
        ...
        int alg_id = 5;
        print_alg_start(alg_id);
        Preference_Learning(p_set, P0, w, alg_id, max_round);
        print_alg_end(alg_id);
    }
