...
namespace sampling{
    ...
    */
    ...
    int sampling(std::vector<point_t *> p_set, point_set_t *P0, int k, int w, int select_opt, const int display_mode, int alg_id){
        reset_stats();
        int num_sample = 250;
        int dim = p_set[0]->dim;
        ...
        while(points_return.size() > w){
            ...
            stop_timer();
            int opt = show_to_user(P0, p1->id, p2->id, display_mode);
            start_timer();
        ...
        stop_timer();
        print_result_list(P0, points_return, display_mode);
