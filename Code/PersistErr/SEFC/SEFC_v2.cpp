...
namespace se_fc_v2{
    ...
    */
    bool preference_exist(const std::set<std::pair<point_t *, point_t *>> &known_preferences, point_t *p1, point_t *p2){
    ...
    */
     ...
     *               Using lazy updating trick
     */
    int find_best_hyperplane_cycle_cand(std::vector<item *> &choose_item_set, std::map<int, hyperplane_t *> &hyperplane_candidates, 
                                                const std::vector<conf_region> &conf_regions, const std::set<std::pair<point_t *, point_t *>> &selected_questions,
                                                const std::set<std::pair<point_t *, point_t *>> &cycle_candidates){
        int k = conf_regions.size() - 1;
        std::vector<std::vector<point_t*>> points_in_region;
        std::vector<int> cand_to_be_removed;
        for(int i=0; i<=k; ++i){
        ...
        for(auto c : hyperplane_candidates){
            ...
            bool is_cycle_cand = false;
            for(auto cc : cycle_candidates){
                if(cc.first == c.second->point1 || cc.second == c.second->point2
                    || cc.first == c.second->point2 || cc.second == c.second->point1){
                    is_cycle_cand = true;
                    break;
                }
            }
            if(!is_cycle_cand) continue;
    ...
    std::pair<point_t *, point_t *> decide_next_cycle_question(point_t *p_i, point_t *p_j, std::set<std::pair<point_t *, point_t *>> &cycle_candidates){
        std::pair<point_t *, point_t *> res;
        for(auto it = cycle_candidates.begin(); it != cycle_candidates.end(); it++){
            point_t *p1 = (*it).first, *p2 = (*it).second;
            if(p1 == p_i || p2 == p_j
                || p1 == p_j || p2 == p_i){
                if(p1 == p_i || p1 == p_j){
                    res.first = p2;
                    res.second = (p1 == p_i) ? p_j : p_i;
                }
                else{
                    res.first = p1;
                    res.second = (p2 == p_i) ? p_j : p_i;
                }
                cycle_candidates.erase(it);
            }
        }
        return res;
    }
    ...
    */
    ...
    /** @brief optimal algorithm but use exact to maintain data structures
     *  
    */
    int Sefc_v2(std::vector<point_t *> p_set, point_t *u, int k, int w, int select_opt, double theta, int beta){
        ...
        std::set<std::pair<point_t *, point_t *>> selected_questions, cycle_candidates;
        
        int round = 0;
        int beta_count = 0;
        bool strategy_fail = false;
        point_t *p_i = 0, *p_j = 0;
        start_timer();
        while(points_return.size() > w){
            point_t *p1 = 0, *p2 = 0;
            halfspace_t *hs = 0;
            if(!strategy_fail && !only_Rk_nonempty(conf_regions)){ // run stage 1
                if(beta_count < beta){ // use SE
                    beta_count++;
                    ...
                    cycle_candidates.insert(make_pair(p1, p2));
                }
                else{ // try finding a cycle
                    if(p_i == 0){
                        int best_idx = find_best_hyperplane_cycle_cand(choose_item_set, hyperplane_candidates, conf_regions, selected_questions, cycle_candidates);
                        if(best_idx <= 0){
                            strategy_fail = true;
                            continue;
                        }
                        ...
                        // set p_i and p_j
                        std::pair<point_t *, point_t *> next_question_pair = decide_next_cycle_question(p1, p2, cycle_candidates);
                        p_i = next_question_pair.first, p_j = next_question_pair.second;
                    }
                    else{
                        beta_count = 0;
                        if(!preference_exist(selected_questions, p_i, p_j)){
                            p1 = p_i, p2 = p_j;
                            p_i = 0, p_j = 0;
                        }
                        else{
