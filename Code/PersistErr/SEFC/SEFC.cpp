...
namespace se_fc{
    ...
    */
     ...
     *               Using lazy updating trick
     */
    int find_best_hyperplane_sefc_strategy_2(std::vector<item *> &choose_item_set, std::map<int, hyperplane_t *> &hyperplane_candidates, 
                                                const std::vector<conf_region> &conf_regions, const std::set<std::pair<point_t *, point_t *>> &selected_questions,
                                                    point_t *p_i, point_t *p_j){
        int k = conf_regions.size() - 1;
        std::vector<std::vector<point_t*>> points_in_region;
        std::vector<int> cand_to_be_removed;
        for(int i=0; i<=k; ++i){
        ...
        for(auto c : hyperplane_candidates){
            ...
            point_t *p1 = c.second->point1, *p2 = c.second->point2;
            point_t *p_k = NULL, *point_asked = NULL;
            if(p1 == p_i || p2 == p_i) point_asked = p_i;
            else if(p1 == p_j || p2 == p_j) point_asked = p_j;
            if(point_asked == NULL) continue;

            p_k = (p1 == point_asked) ? p2 : p1;
            if(point_asked == p_i){
                if(preference_exist(selected_questions, p_j, p_k)){
                    cand_to_be_removed.push_back(c.first);
                    continue; 
                }
            }
            else{
                if(preference_exist(selected_questions, p_i, p_k)){
                    cand_to_be_removed.push_back(c.first);
                    continue; 
                }
            }
    ...
    int Sefc_alg(std::vector<point_t *> p_set, point_t *u, int k, int w, int select_opt, double theta){
        ...
        while(points_return.size() > w){
            ...
            if(!strategy_fail && !only_Rk_nonempty(conf_regions)){ // run stage 1
                ...
                else if(round % 3 == 1){ // use strategy 2
                    int best_idx = find_best_hyperplane_sefc_strategy_2(choose_item_set, hyperplane_candidates, conf_regions, selected_questions, p_i, p_j);
                    if(best_idx <= 0){
                        strategy_fail = true;
                        continue;
                    }
