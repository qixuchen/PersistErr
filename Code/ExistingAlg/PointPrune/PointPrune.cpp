...
int PointPrune_v2(std::vector<point_t *> p_set, point_t *u, int checknum, int w, double theta)
{
    ...
    while(considered_half_set_cp.size() > w)
    {
        point_result = NULL;
        ...
        while(considered_half_set_cp.size() > w && selected_halfspaces.size()>0)
        {
...
int PointPrune_Persist(std::vector<point_t *> p_set, point_t *u, int checknum, int w, double theta)
{
    ...
    while(considered_half_set_cp.size() > w)
    {
        ...
        while(considered_half_set_cp.size() > w && selected_halfspaces.size()>0)
        {
            ...
            if(best_p1==p1){
                //user_choice = checking(u,p2,p1,theta,checknum);
                user_choice = checking_varyk_persist(u, p2, p1, theta, checknum, round);
            }
            else{
                //user_choice = checking(u,p1,p2,theta,checknum);
                user_choice = checking_varyk_persist(u, p1, p2, theta, checknum, round);
            }
            //printf("ratio %10f\n",ratio);
            if(user_choice!=best_p2){
                encounter_err = true;
